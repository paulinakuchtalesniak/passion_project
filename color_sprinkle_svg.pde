...
int frameCount = 0; // Initialize frame counter
...
void setup() {
  ...
  // Initialize PGraphics with transparency
...
background(30);
  pg.beginDraw();
  pg.clear(); // Clear the PGraphics to prevent overlapping drawings
  pg.background(0, 0, 0, 0); // Set a transparent background
 ...
 if (showCircles) drawControlCirclesOnGraphics(pg);
  ...
  // Display the PGraphics on the screen
  image(pg, 0, 0);

  // Save frame if recording
  if (isRecording) {
    pg.save("frames/frame-" + frameCount + ".png"); // Save without leading zeros
...
void keyPressed() {
  ...
  else if (key == 'E' || key == 'e') { // Dodajemy warunek dla klawisza 'E'
    currentLetter = "E";
    initializeLetterE();
    circlePositions = generateCirclePositions();
    generateAllEdgesWithDeformation();
...
void generateAllEdgesWithDeformation() {
  ...
  edgesWithDeformations.add(generateDeformedEdge(eBackbonePositions[0], eBackbonePositions[1]));
    ...
    // Generate edges for the letter "F"
    edgesWithDeformations.add(generateDeformedEdge(eBackbonePositions[0], eBackbonePositions[1]));
    ...
    // Generate edges for the letter "J"
    ...
    // Generate edges for the letter "I"
    ...
    // Add vertical lines to connect top and bottom
    ...
    // Generate edges for the letter "N"
    ...
    // Generate edges for the outer part of the letter "D"
    ...
    // Generate edges for the inner part of the letter "D"
...
void mouseDragged() {
  if (selectedCircle != -1 && selectedCircle < circlePositions.length) {
    ...
    generateAllEdgesWithDeformation(); // Regenerate edges after dragging
  }
}
...
int[][] generateCirclePositions() {
    ...
    if (currentLetter.equals("A")) {
        ...
        positions = new int[0][0]; // Default initialization
    }
...
void initializeLetterE() {
    ...
    float stemWidth = random(20, 40); // Randomized width
    float horizontalCenterLineY = 50; // Example value, adjust as needed
    float shortEnd = 250 + random(0, 50); // Ensure positive direction

    float middleY = capHeightY + ((baselineY - capHeightY) * (horizontalCenterLineY / 100.0f)) / 1.7f;
    float longEnd = 350 + random(0, 100); // Randomized within range
    ...
    // Initialize middle horizontal end
...
void initializeLetterJ() {
    ...
    // Define positions with some randomness
    jTopPoints[0][0] = (int)(canvasSizePart * 6 - stemWidth + random(-10, 10)); // 1
    jTopPoints[0][1] = (int)capHeightY;
    
    jTopPoints[1][0] = (int)(canvasSizePart * 6 + random(-10, 10)); // 2
    jTopPoints[1][1] = (int)capHeightY;
    
    jStemEnd[0] = jTopPoints[1][0]; // 3
    jStemEnd[1] = 440 + (int)random(-10, 10);
    
    jCurveEnd[0] = jTopPoints[1][0] - (int)stemWidth * 2 + (int)random(-10, 10); // 4
    jCurveEnd[1] = (int)baselineY;
    
    jCurveControl[0] = jTopPoints[1][0] - (int)stemWidth * 4 + (int)random(-10, 10); // 5
    jCurveControl[1] = (int)baselineY;
    
    jCurveControl2[0] = jTopPoints[1][0] - (int)stemWidth * 6 + (int)random(-10, 10); // 6
    jCurveControl2[1] = 450 + (int)random(-10, 10);
    
    jConnectingPoint[0] = jTopPoints[1][0] - (int)stemWidth * 7 + (int)random(-10, 10); // 7
    jConnectingPoint[1] = 400 + (int)random(-10, 10);
    
    jInnerCurveControl[0] = jConnectingPoint[0] + (int)stemWidth + (int)random(-5, 5); // 8
    jInnerCurveControl[1] = 400 + (int)random(-5, 5);
    
    jInnerCurveControl2[0] = jInnerCurveControl[0] + (int)stemWidth + (int)random(-5, 5); // 9
    jInnerCurveControl2[1] = 430 + (int)random(-5, 5);
    
    point10[0] = jInnerCurveControl2[0] + (int)stemWidth * 2 + (int)random(-5, 5); // 10
    point10[1] = 460 + (int)random(-5, 5);
    
    point11[0] = jTopPoints[1][0] - (int)stemWidth + (int)random(-5, 5); // 11
...
void initializeLetterI() {
    ...
    float baselineY = 500;
    float canvasSizePart = canvasSize / 10.0f;
    float stemWidth = 20; // Example width, adjust as needed
...
void initializeLetterN() {
    ...
    float startX = 150; // Start at 150
    float endX = 450; // End at 450
